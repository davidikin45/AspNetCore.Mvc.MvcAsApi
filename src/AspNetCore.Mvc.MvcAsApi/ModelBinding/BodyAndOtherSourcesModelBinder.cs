...
namespace AspNetCore.Mvc.MvcAsApi.ModelBinding
{
    ...
    public class BodyAndOtherSourcesModelBinder : IModelBinder
    {
        ...
        public async Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            var bodyBinder = GetBinder(bindingContext.ModelMetadata, BindingSource.Body);
            ...
            if (bodyBinder != null)
            {
                ...
                if ((bindingContext.BindingSource == null || !bindingContext.BindingSource.CanAcceptDataFrom(BindingSource.Body)))
                {
                    ...
                    if (!bindingContext.Result.IsModelSet)
                    {
                        ...
                        _logger.LogDebug("Couldn't bind from Body, now binding from BindingSource: {0}", bindingContext.BindingSource != null ? bindingContext.BindingSource.DisplayName.Replace("BodyAnd", "") : BindingSource.ModelBinding.DisplayName);
                    }
                    else
                    {
                        bindingContext.Model = bindingContext.Result.Model;
                        _logger.LogDebug("Bound from Body, now binding from BindingSource: {0}", bindingContext.BindingSource != null ? bindingContext.BindingSource.DisplayName.Replace("BodyAnd", "") : BindingSource.ModelBinding.DisplayName);
                    }
                }
            }
            ...
            {
                var binder = GetBinder(bindingContext.ModelMetadata, bindingContext.BindingSource ?? BodyOrBindingSource.BodyOrModelBinding);
